...
namespace Arma.Studio.SolutionExplorer
{
    public class PluginMain : IPlugin, IDockableProvider
    {
        ...
        public string Name => Properties.Language.SolutionExplorer_Name;
        ...
        public IEnumerable<DockableInfo> Dockables => new DockableInfo[] {
            DockableInfo.Create(Properties.Language.SolutionExplorer, ECreationMode.Anchorable, () => new SolutionExplorerDataContext())
        };
        public void AddDataTemplates(GenericDataTemplateSelector selector)
        {
            selector.AddAllDataTemplatesInAssembly(typeof(PluginMain).Assembly, (s) => s.StartsWith("Arma.Studio.SolutionExplorer"));
        }
        #endregion
    }
}
