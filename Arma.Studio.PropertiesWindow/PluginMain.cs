...
namespace Arma.Studio.PropertiesWindow
{
    public class PluginMain : IPlugin, IDockableProvider
    {
        ...
        public string Name => Properties.Language.PropertiesWindow_Name;
        ...
        public IEnumerable<DockableInfo> Dockables => new DockableInfo[] {
            DockableInfo.Create(Properties.Language.PropertiesWindow, ECreationMode.Anchorable, () => new PropertiesWindowDataContext())
        };
        public void AddDataTemplates(GenericDataTemplateSelector selector)
        {
            selector.AddAllDataTemplatesInAssembly(typeof(PluginMain).Assembly, (s) => s.StartsWith("Arma.Studio.PropertiesWindow"));
        }
        #endregion
    }
}
